...
public function addList ($listName, $description, $templateID ) {
		...
		try {
			$result = $this->xmlHandler($this->soapClient->AddList($xmlvar)->AddListResult->any);
		} catch (\SoapFault $fault) {
			$this->onError($fault);
		}
...
public function ModifyListFields ($CAML ) {  // Replace on params in future 
...
#var_dump($xmlvar);
		// Attempt to run operation
		try {
			$result = $this->xmlHandler($this->soapClient->UpdateList($xmlvar)->UpdateListResult->any);
		} catch (\SoapFault $fault) {
			$this->onError($fault);
		}
...
public function addFieldsHR ($listName ) {  // Replace on params in future 
                        ...
                        <newFields>
                                <Fields>
                                 <Method ID="10" AddToView="">
                                 	<Field DisplayName="HRID" Type="Text"></Field>
        	                 </Method>
...
public function AddUserToGroup ($groupName,$login) {
		// Ready XML
		$CAML = '<AddUserToGroup xmlns="http://microsoft.com/webservices/SharePointPortalServer/UserProfileService">
            		<groupName>'.$groupName.'</groupName>
					<userLoginName>'.$login.'</userLoginName>
        	</AddUserToGroup>
		';
		try {
		$propObj = $this->soapClient->AddUserToGroup(new \SoapVar($CAML, XSD_ANYXML))->AddUserToGroupResult->PropertyData;
		} catch (\SoapFault $fault) {
			return $results;
//			$this->onError($fault);
		}
...
public function GetUserProfileByName ($login, $hideInternal = TRUE, $ignoreHiddenAttribute = FALSE) {
		// Ready XML
		$CAML = '<GetUserProfileByName xmlns="http://microsoft.com/webservices/SharePointPortalServer/UserProfileService">
            		<AccountName>'.$login.'</AccountName>
        	</GetUserProfileByName>
		';

#            		<AccountName>KYIVSTAR.UA'."\\".$login.'</AccountName>
		// Attempt to query Sharepoint
#var_dump("\n--------\n".$CAML);
		$rawXml = '';
#echo("\n----------\n");
#var_dump($this->soapClient->GetUserProfileByName(new \SoapVar($CAML, XSD_ANYXML))->GetUserProfileByNameResult);

		$results= array();
		$results['PictureURL']='';
		$results['UserProfile_GUID']='';
		$results['AccountName']='';
		$results['LastName']='';
		$results['Manager']='';
		if (strlen($login)<1){
		...
		try {
		$propObj = $this->soapClient->GetUserProfileByName(new \SoapVar($CAML, XSD_ANYXML))->GetUserProfileByNameResult->PropertyData;
		} catch (\SoapFault $fault) {
			return $results;
//			$this->onError($fault);
		}
		foreach ($propObj as $counter => $prop) {
			#var_dump($prop->Values);
			//if (property_exists($prop,'Values') AND property_exists($prop->Values,'ValueData') AND property_exists($prop->Values->ValueData,'Value')){
			if (property_exists($prop,'Values')){
				if (property_exists($prop->Values,'ValueData')){
					if (property_exists($prop->Values->ValueData,'Value')){
						if ($prop->Name=='UserProfile_GUID'){
		#					var_dump($prop->Values->ValueData->Value->enc_value);
							$results[$prop->Name]=$prop->Values->ValueData->Value->enc_value;
						}else{
							$results[$prop->Name]=$prop->Values->ValueData->Value;
						}
...
#file_put_contents("/home/jive/sync_SP/log_".$method.date('ymd-His').rand(10,99).".txt", $CAML,FILE_APPEND);
